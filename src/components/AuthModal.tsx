...
import { Lock, Mail, User, Phone, Eye, EyeOff, X, ArrowRight, UserCheck, MapPin } from 'lucide-react';
...
export const AuthModal: React.FC<AuthModalProps> = ({
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    if (!/^01[3-9]\d{8}$/.test(signUpPhone.trim().replace(/\D/g, '').slice(-11)) && !/^8801[3-9]\d{8}$/.test(signUpPhone.trim().replace(/\D/g, ''))) {
      setError('Please enter a valid Bangladeshi mobile number (e.g. 01712345678).');
      return;
    }
    if (!signUpAddress.trim()) {
      setError('Delivery address is required to create an account.');
      return;
    }
    if (signUpAddress.trim().length < 10) {
      setError('Please write your full delivery address (house, road, area, district).');
      return;
    }
            ...
            <form onSubmit={handleSignUp} className="space-y-3.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 mt-1">
                  বাংলাদেশি মোবাইল নম্বর দিন — অর্ডার কনফার্মেশন ও ডেলিভারি কল এই নম্বরে যাবে।
                </p>
              </div>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <textarea
                    ...
                    placeholder="House / flat, road, area, thana, district — e.g. House 12, Road 7, Dhanmondi, Dhaka"
                    rows={2}
                    required
                    className="w-full pl-10 pr-3.5 py-2 rounded-xl border border-slate-300 text-xs text-slate-900 focus:border-amber-500 focus:ring-2 focus:ring-amber-500/20 outline-none resize-none"
                  />
                </div>
                <p className="text-[10px] text-slate-400 mt-1">
                  Full address required — courier pickup ও cash on delivery-র জন্য ব্যবহার হবে।
                </p>
              </div>
