...
import { CartItem, DeliveryZone, Order, PaymentMethod, StoreSettings, UserAccount } from '../types';
import { formatBDT, getWhatsAppUrl, openWhatsAppChat, buildCartWhatsAppMessage } from '../utils/helpers';
...
interface CheckoutModalProps {
  ...
  /** Signed-in customer – their saved profile pre-fills the delivery form. */
...
export const CheckoutModal: React.FC<CheckoutModalProps> = ({
  ...
  // Pre-fill from the signed-in customer's saved profile (never overwrites typing)
  useEffect(() => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    // Hard guard against double submits (double click / Enter key) – one checkout = exactly one order
    ...
    try {
      ...
      // (Cloud persistence happens inside api.createOrder – no duplicate write needed here)
