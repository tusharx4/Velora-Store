        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          <div className="glass-panel-strong rounded-full px-4 sm:px-6 py-2.5 flex items-center justify-between gap-3 shadow-md">
            ...
            <div className="flex items-center gap-2 sm:gap-2.5">
              {/* My Account – opens the profile & order history page */}
              <button
                onClick={() => (currentUser ? onNavigate('account') : onOpenAuth('signin'))}
                title="My Account & Orders"
                className={`hidden sm:inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-medium transition-all border cursor-pointer ${
                  currentRoute === 'account'
                    ? 'bg-[#12151f] text-amber-300 border-[#12151f] shadow-xs'
                    : 'text-gray-700 hover:bg-black/5 border-transparent hover:border-black/10'
                }`}
              >
              ...
              {/* Cart Button – navigates to the dedicated cart page (replaces the old side drawer) */}
              <button
                onClick={() => onNavigate('cart')}
                className={`relative p-2.5 rounded-full shadow-sm border hover:scale-105 active:scale-95 transition-all flex items-center justify-center cursor-pointer ${
                  currentRoute === 'cart'
                    ? 'bg-[#12151f] text-amber-300 border-[#12151f]'
                    : 'bg-white/80 hover:bg-white text-gray-900 border-white/90'
                }`}
