...
export const AdminOrders: React.FC<AdminOrdersProps> = ({ orders, onRefresh }) => {
  ...
  const allCustomerOrders = safeOrders.filter(
    (o) =>
      o.customerPhone.replace(/[^0-9]/g, '') ===
        selectedOrder?.customerPhone.replace(/[^0-9]/g, '') ||
  ...
  const handleWhatsAppCustomer = (order: Order) => {
    ...
    window.open(getWhatsAppUrl(order.customerPhone, msg), '_blank', 'noopener,noreferrer');
  };
