...
/** Reusable checklist of admin-panel areas a moderator may be granted. */
const PermissionPicker: React.FC<{
  value: ModeratorPermission[];
  onChange: (next: ModeratorPermission[]) => void;
}> = ({ value, onChange }) => {
  const toggle = (id: ModeratorPermission) =>
    onChange(value.includes(id) ? value.filter((p) => p !== id) : [...value, id]);
  const allIds = MODERATOR_PERMISSIONS.map((p) => p.id);

  return (
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider">
          {value.length} of {allIds.length} areas selected
        </span>
        <div className="flex gap-2 text-[11px] font-semibold">
          <button type="button" onClick={() => onChange(allIds)} className="text-indigo-600 hover:underline cursor-pointer">
            Select all
          </button>
          ...
          return (
            ...
            >
              ...
              <span className="min-w-0">
                ...
                <span className="block text-[10px] font-bn text-slate-500">{perm.bn}</span>
                <span className="block text-[11px] text-slate-500 mt-0.5 leading-snug">{perm.desc}</span>
              </span>
            </button>
          );
        })}
      ...
      <p className="text-[11px] text-slate-400">
        Users &amp; Roles, Store Settings and Reset Store always remain admin-only.
      </p>
    </div>
  );
};
...
export const AdminUsers: React.FC<AdminUsersProps> = ({ currentUser, onRefreshUser }) => {
  ...
  const [staffPermissions, setStaffPermissions] = useState<ModeratorPermission[]>(DEFAULT_MODERATOR_PERMISSIONS);

  // Edit Permissions Modal
  const [permissionTarget, setPermissionTarget] = useState<UserAccount | null>(null);
  const [permissionDraft, setPermissionDraft] = useState<ModeratorPermission[]>([]);

  const openPermissions = (user: UserAccount) => {
  ...
  const handleSavePermissions = async () => {
    ...
    try {
      ...
      const res = await api.setUserPermissions(permissionTarget.email, permissionDraft);
      showNotification('success', res.message);
      setPermissionTarget(null);
      loadUsers();
    } catch (err: any) {
      showNotification('error', err.message || 'Failed to update permissions.');
    } finally {
  ...
  const handleToggleModerator = async (user: UserAccount) => {
    ...
    try {
      ...
      loadUsers();
      if (onRefreshUser) onRefreshUser();
      // Newly promoted → let the admin pick the areas straight away
      if (nextRole === 'moderator') openPermissions(res.user);
    } catch (err: any) {
  ...
  const handleCreateStaff = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.createStaffUser({
        ...
        role: staffRole,
        permissions: staffRole === 'moderator' ? staffPermissions : undefined,
      });
      showNotification('success', res.message);
      ...
      setStaffPhone('');
      setStaffPassword('');
      setStaffPermissions(DEFAULT_MODERATOR_PERMISSIONS);
      loadUsers();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div className="bg-white rounded-2xl p-5 border border-slate-200 shadow-xs">
          ...
          <div className="text-[11px] text-slate-500 mt-1">
            Each moderator only sees the areas you grant them
          </div>
        </div>
          ...
          <div className="space-y-2 flex-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-xs text-slate-300">
              ...
              <div className="p-2.5 rounded-xl bg-white/5 border border-white/10 space-y-1">
                ...
                <p className="text-[11px] text-slate-300 leading-relaxed">
                  Custom access per person — choose from Orders, Customer Support, Flash Sales, Products, Categories, Banners, AI tools and Analytics. Nothing else is visible to them.
                </p>
              </div>
                          ...
                          <div className="space-y-1.5">
                            ...
                            <div className="flex flex-wrap gap-1 max-w-xs">
                              {(user.permissions || []).length === 0 ? (
                                <span className="text-[10px] text-rose-600 font-semibold">No areas assigned</span>
                              ) : (
                                (user.permissions || []).map((p) => (
                                  <span key={p} className="text-[10px] px-1.5 py-0.5 rounded bg-slate-100 text-slate-700 border border-slate-200">
                                    {MODERATOR_PERMISSIONS.find((m) => m.id === p)?.label || p}
                                  </span>
                                ))
                              )}
                            </div>
                      ...
                      <td className="py-3.5 px-4 text-right">
                        {isUserPrimary ? (
                          ...
                          <div className="flex items-center justify-end gap-2">
                            {isPrimaryAdmin && user.role === 'moderator' && (
                              <button
                                onClick={() => openPermissions(user)}
                                disabled={actionLoading}
                                className="px-2.5 py-1 rounded-lg text-xs font-semibold bg-indigo-50 text-indigo-700 hover:bg-indigo-100 border border-indigo-200 transition-all cursor-pointer flex items-center gap-1.5"
                                title="Choose which areas this moderator can access"
                              >
              ...
              {staffRole === 'moderator' && (
                <div className="pt-1">
                  <label className="block text-xs font-semibold text-slate-700 mb-2">
                    Areas this moderator can access
                  </label>
                  <div className="max-h-64 overflow-y-auto pr-1">
                    <PermissionPicker value={staffPermissions} onChange={setStaffPermissions} />
                  </div>
                </div>
              )}
      ...
      {/* Modal: Moderator Permissions */}
      {permissionTarget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/60 backdrop-blur-xs animate-fade-in">
          <div className="relative w-full max-w-2xl bg-white rounded-3xl shadow-2xl border border-slate-200 max-h-[90vh] flex flex-col">
            <div className="p-6 sm:p-7 pb-4 border-b border-slate-100">
              ...
              <p className="text-xs text-slate-500 mt-1">
                Choose exactly what <strong className="text-slate-800">{permissionTarget.name}</strong> ({permissionTarget.email}) can open in the admin panel.
                Changes apply within about 30 seconds — no sign-out needed.
              </p>
            </div>

            <div className="p-6 sm:p-7 py-5 overflow-y-auto">
              <PermissionPicker value={permissionDraft} onChange={setPermissionDraft} />
            </div>

            <div className="p-5 sm:px-7 border-t border-slate-100 flex items-center justify-end gap-2.5">
              <button
                type="button"
              ...
              <button
                ...
                disabled={actionLoading}
                className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl shadow-sm transition-all cursor-pointer disabled:opacity-50"
              >
