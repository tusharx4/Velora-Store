...
async function seedCatalog(source: CatalogSource, force: boolean): Promise<void> {
  const [prods, cats, bans, settings] = force
    ? [[], [], [], await fetchOne<StoreSettings>(COL.SETTINGS, SETTINGS_DOC_ID)]
    : await Promise.all([
...
function ensureCatalogSeeded(): Promise<void> {
  if (!seedPromise) {
    seedPromise = (async () => {
      const meta = await fetchOne<CatalogMeta>(COL.SETTINGS, META_DOC_ID);
    ...
    seedPromise.catch(() => {
      seedPromise = null; // allow a retry later
    });
  }
  return seedPromise;
}
