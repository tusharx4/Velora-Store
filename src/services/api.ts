...
/** Legacy moderator accounts (created before granular permissions existed) receive the default area set once. */
async function backfillModeratorPermissions(user: CloudUser, cloudAvailable: boolean): Promise<CloudUser> {
  if (user.role !== 'moderator' || (Array.isArray(user.permissions) && user.permissions.length > 0)) {
    return user;
  }
  const updated: CloudUser = { ...user, permissions: [...DEFAULT_MODERATOR_PERMISSIONS] };
  upsertLocalUser(updated);
  if (cloudAvailable) {
    putDoc(COL.USERS, updated.id, updated).catch(() => {});
  }
  return updated;
}
...
function getLocalOrders(): Order[] {
  ...
  return (Array.isArray(parsed) ? parsed : [...INITIAL_ORDERS]).map(normalizeOrder);
}
...
const VALID_PERMISSIONS = new Set<string>(MODERATOR_PERMISSIONS.map((p) => p.id));
function cleanPermissions(list: unknown): ModeratorPermission[] {
  if (!Array.isArray(list)) return [];
  return Array.from(new Set(list.filter((p): p is ModeratorPermission => typeof p === 'string' && VALID_PERMISSIONS.has(p))));
}
...
function sanitizeUser(user: CloudUser | UserAccount): UserAccount {
  return {
    ...
    avatar: user.avatar,
    permissions: user.role === 'moderator' ? cleanPermissions(user.permissions) : undefined,
    address: user.address,
  };
}

/** Bangladeshi mobile check: 01X-XXXXXXX (11 digits) with optional +88 / 88 prefix. */
...
/** Stub docs (created by old merge-patch writes) carry no phone and no items – they must never surface as orders. */
const isStubOrder = (o: Order): boolean => (o.items || []).length === 0 && !o.customerPhone;

/** Field-completeness score used when the same order id exists both locally and in the cloud. */
function orderRichness(o: Order): number {
  return (
    ((o.items || []).length > 0 ? 4 : 0) +
    (o.customerPhone ? 2 : 0) +
    (o.customerName && o.customerName !== 'Guest Customer' ? 2 : 0) +
    (o.address ? 1 : 0) +
    (o.total ? 1 : 0)
  );
}
...
async function loadOrders(): Promise<Order[]> {
  ...
  if (!hasStaffSession()) return local;
  try {
    const cloud = (await fetchAll<Order>(COL.ORDERS)).map(normalizeOrder);

    // Union local + cloud by id; for duplicates keep the richer record (newer updatedAt as tie-break)
    ...
    const push = (o: Order) => {
      ...
      const score = orderRichness(o);
      const existingScore = orderRichness(existing);
      if (score > existingScore || (score === existingScore && (o.updatedAt || '') > (existing.updatedAt || ''))) {
        byId.set(o.id, o);
      }
    };
    local.forEach((o) => !isStubOrder(o) && push(o));
    cloud.forEach((o) => !isStubOrder(o) && push(o));

    // Repair or remove stub docs in the cloud so the phantom "Guest Customer" orders disappear everywhere
    cloud.filter(isStubOrder).forEach((stub) => {
      const rich = local.find((o) => o.id === stub.id && !isStubOrder(o));
      if (rich) putDoc(COL.ORDERS, stub.id, rich).catch(() => {});
      else removeDoc(COL.ORDERS, stub.id).catch(() => {});
    });

    const merged = sortOrdersDesc(Array.from(byId.values()));
    saveLocalOrders(merged);
    return merged;
  } catch {
    return sortOrdersDesc(local);
  }
}
...
/**
 * Orders written by older builds / manual Firestore docs can miss fields.
 * Normalising at the source keeps every screen (dashboard, orders table,
 * tracking modal) crash-free.
 */
export function normalizeOrder(o: Order): Order {
  ...
  return {
    ...
    deliveryZone: o.deliveryZone || 'dhaka',
...
/** Pure analytics computation – shared by the async (cloud) and instant (offline) paths. */
function buildAnalytics(orders: Order[], products: Product[]): AnalyticsSummary {
  ...
  const lowStockProducts = safeProducts.filter((p) => p.stock > 0 && p.stock <= 10).length;
  ...
  const categoryBySlug = new Map(safeProducts.map((p) => [p.slug, p.cat]));
  ...
  return {
    ...
    outOfStockProducts,
    averageOrderValue,
    flashSaleProductsCount: safeProducts.filter((p) => p.flashSale).length,
    categoryBreakdown,
    ...
    statusBreakdown,
    recentOrders: sortOrdersDesc(safeOrders).slice(0, 8),
  };
}
...
export const api = {
  ...
  async login(
    ...
    user = await backfillModeratorPermissions(user, !!cloudUsers);
  ...
  async register(
    ...
    // Mobile number and delivery address are mandatory for every new account
    if (!cleanPhone) throw new Error('Mobile number is required to create an account.');
    if (!isValidBdPhone(cleanPhone)) {
      throw new Error('Please enter a valid Bangladeshi mobile number (e.g. 01712345678).');
    }
    if (!cleanAddress) throw new Error('Delivery address is required to create an account.');
    if (cleanAddress.length < 10) {
      throw new Error('Please write your full delivery address (house, road, area, district).');
    }
  ...
  async fetchCurrentUser(): Promise<UserAccount | null> {
    ...
    const withPerms = await backfillModeratorPermissions(live, true);
    const refreshed = sanitizeUser(withPerms);
  ...
  async updateOrderStatus(id: string, status: OrderStatus, trackingNumber?: string): Promise<Order> {
    ...
    const updated: Order = normalizeOrder({ ...base, ...patch });
    ...
    try {
      // Full-document write (never a bare merge patch) so no field-less stub docs can appear in Firestore
      await putDoc(COL.ORDERS, updated.id, updated);
    } catch (err) {
  ...
  async trackOrder(queryText: string): Promise<Order[]> {
    ...
    return sortOrdersDesc(Array.from(results.values()).map(normalizeOrder));
  },

  // ------------------------------------------------------
  // CUSTOMER ACCOUNT
  // ------------------------------------------------------
  /** Every order that belongs to this customer (phone tail or email), local + cloud merged. */
  async getCustomerOrders(user: UserAccount): Promise<Order[]> {
    const phoneTail = last10(user.phone || '');
    const email = (user.email || '').toLowerCase();
    const matches = (o: Order) =>
      (!!phoneTail && last10(o.customerPhone || '') === phoneTail) ||
      (!!email && (o.customerEmail || '').toLowerCase() === email);

    let cloud: Order[] = [];
    try {
      cloud = (await fetchAll<Order>(COL.ORDERS)).map(normalizeOrder);
    } catch {
      /* offline or rules blocked – local history still shows */
    }

    const byId = new Map<string, Order>();
    [...getLocalOrders(), ...cloud].forEach((o) => {
      if (matches(o) && !byId.has(o.id)) byId.set(o.id, o);
    });
    return sortOrdersDesc(Array.from(byId.values()));
  },

  /** Customer edits their own name / mobile / delivery address. */
  async updateProfile(
    userId: string,
    ...
    const cloudUsers = await loadCloudUsers();
    const pool = cloudUsers ?? getLocalUsers();
    const target = pool.find((u) => u.id === userId) || (session ? pool.find((u) => u.id === session.id) : undefined);
    if (!target) throw new Error('Account not found. Please sign in again.');

    const name = (patch.name ?? target.name ?? '').trim();
    const phone = (patch.phone ?? target.phone ?? '').trim();
    const address = (patch.address ?? target.address ?? '').trim();

    if (!name) throw new Error('Name cannot be empty.');
    if (!phone) throw new Error('Mobile number is required.');
    if (!isValidBdPhone(phone)) throw new Error('Please enter a valid Bangladeshi mobile number (e.g. 01712345678).');
    if (!address) throw new Error('Delivery address is required.');
    if (address.length < 10) throw new Error('Please write your full delivery address (house, road, area, district).');

    const updated: CloudUser = { ...target, name, phone, address };
    upsertLocalUser(updated);
    try {
      await putDoc(COL.USERS, updated.id, updated);
    } catch (err) {
      throw new Error(cloudErrorMessage('Profile update', err));
    }

    const sanitized = sanitizeUser(updated);
    if (session && session.id === updated.id) writeSession(sanitized);
    return sanitized;
  },
  ...
  async getAnalytics(): Promise<AnalyticsSummary> {
    ...
    return buildAnalytics(orders, products);
  },

  /** Instant offline analytics – lets the dashboard paint immediately, refined later from the cloud. */
  computeAnalyticsSync(orders: Order[], products: Product[]): AnalyticsSummary {
    return buildAnalytics(orders, products);
  },
  ...
  async assignUserRole(
    ...
    if (role === 'moderator') {
      updated.permissions = cleanPermissions(
        permissions ?? (target.permissions?.length ? target.permissions : DEFAULT_MODERATOR_PERMISSIONS)
      );
    } else {
  ...
  /** Admin: set exactly which admin-panel areas a moderator may use. Takes effect on the moderator's next refresh. */
  async setUserPermissions(
    identifier: string,
    permissions: ModeratorPermission[]
  ): Promise<{ success: boolean; message: string; user: UserAccount }> {
    if (readSession()?.role !== 'admin') throw new Error('Only an administrator can change permissions.');
    const cloudUsers = await loadCloudUsers();
    const pool = cloudUsers ?? getLocalUsers();
    const target = findUserMatch(pool, identifier);
    if (!target) throw new Error('User not found.');
    if (target.role !== 'moderator') throw new Error('Permissions can only be set for moderators.');

    const updated: CloudUser = { ...target, permissions: cleanPermissions(permissions) };
    upsertLocalUser(updated);
    try {
      await putDoc(COL.USERS, updated.id, updated);
    } catch (err) {
      throw new Error(cloudErrorMessage('Permission update', err));
    }
    const count = updated.permissions?.length || 0;
    return {
      success: true,
      message: `${updated.name} now has access to ${count} area${count === 1 ? '' : 's'}.`,
    ...
    const user = await withNewPassword(
      {
        ...
        role: data.role,
        permissions: data.role === 'moderator' ? cleanPermissions(data.permissions ?? DEFAULT_MODERATOR_PERMISSIONS) : undefined,
        createdAt: nowIso(),
      },
      raw
    );
